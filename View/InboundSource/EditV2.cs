...
namespace Warehouse_IO.View.InboundSource
{
    public partial class EditV2 : AddEdit_Inbound_Outbound_FormV2
    {
        ...
        //Variable to take last Trucklist and Productlist to compare if it's changed
        private Dictionary<Truck, int> oldtrucklist;
        private Dictionary<Product, int> oldproductlist;
        string inv;
        ...
        public EditV2()
        {
            ...
            edit = new Inbound(Global.tempPkey);
            //Duplicate oldlist for compare at Done & Close
            oldtrucklist = edit.TruckQuantityPerShipmentList;
            oldproductlist = edit.QuantityOfProductList;
            inv = edit.InvoiceNo;
        ...
        private void CheckAllThings()
        {
            ...
            if (EditShipment())
            {
                ...
                {
                    MessageBox.Show(this, "Truck can't be blank");
                    return;
                }
                else if (edit.TruckQuantityPerShipmentList.Values.Count > 0 && edit.QuantityOfProductList.Values.Count <= 0)
                {
                    MessageBox.Show(this, "Product can't be blank");
                    return;
                }
                else if (edit.TruckQuantityPerShipmentList.Values.Count <= 0 && edit.QuantityOfProductList.Values.Count <= 0)
                {
                    MessageBox.Show("You want to remove this shipment ?");
                    Global.tempPkey = -1;
                    Global.tempPkey = edit.ID;
                    remove = new Remove();
                    remove.Owner = main;
                    remove.ShowDialog();
