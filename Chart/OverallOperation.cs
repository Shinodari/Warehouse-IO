...
namespace Warehouse_IO.Chart
{
    public partial class ChartDisplayForm : Form
    {
        ...
        private void UpdateTruckInboundChart()
        {
            ...
            {
                if((inbT.Date == from2Timeline.Date && inbT.Date.TimeOfDay < to2Timeline.TimeOfDay) ||
                        (inbT.Date > from2Timeline.Date && inbT.Date <= to2Timeline.Date))
                {
                ...
                else if ((inbT.Date == from1Timeline.Date && inbT.Date.TimeOfDay < from1Timeline.TimeOfDay) ||
                    (inbT.Date > from1Timeline.Date && inbT.Date <= to1Timeline.Date))
                {
                    if (inbT.TypeName == "20F")
                    {
        ...
        private void UpdateTruckOutboundChart()
        {
            ...
            {
                if ((oubT.Date == from2Timeline.Date && oubT.Date.TimeOfDay < to2Timeline.TimeOfDay) ||
                        (oubT.Date > from2Timeline.Date && oubT.Date <= to2Timeline.Date))
                {
                ...
                else if ((oubT.Date == from1Timeline.Date && oubT.Date.TimeOfDay < from1Timeline.TimeOfDay) ||
                    (oubT.Date > from1Timeline.Date && oubT.Date <= to1Timeline.Date))
                {
                    switch(oubT.TypeName)
                    {
                        case "4W":
